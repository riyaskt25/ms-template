...
public class CompanySalesmanInvitationRequest {
  ...
  @Schema(
      description = "Invitee mobile number",
  ...
  @Schema(
      description = "Invitee national or identity number",
  ...
  @Schema(
      description = "Company identifiers to invite against",
      example = "[1001, 1002]",
      requiredMode = Schema.RequiredMode.REQUIRED)
  private List<
          @NotNull(message = "{validation.companySalesmanInvitation.companyIds.notNull}") @Positive(message = "{validation.companySalesmanInvitation.companyIds.positive}") Long>
      companyIds;

  @Future(message = "{validation.companySalesmanInvitation.expiryDate.future}")
  @Schema(
      description =
          "Invitation expiry timestamp. If omitted, the API sets it to 7 days from invitation time.",
