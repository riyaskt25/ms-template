...
public class CompanySalesmanInvitationController implements CompanySalesmanInvitationApi {
  ...
  public ResponseEntity<List<CompanySalesmanInvitationResponse>> create(
    ...
    log.debug(
        "Received request to create salesman invitations companyIds={} emailAddress={}",
        request.getCompanyIds(),
        request.getEmailAddress());
    List<CompanySalesmanInvitationResponse> created =
        companySalesmanInvitationService.create(request);
    log.info(
        "Created {} salesman invitation(s) for companyIds={}",
        created.size(),
        request.getCompanyIds());
