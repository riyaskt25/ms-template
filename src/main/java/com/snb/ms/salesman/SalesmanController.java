...
public class SalesmanController implements SalesmanApi {
  ...
  public SalesmanResponse findById(@PathVariable Long id) {
    ...
    SalesmanResponse result =
        salesmanService.findById(id).orElseThrow(() -> ResourceNotFoundException.salesmanById(id));
  ...
  @Override
  @PutMapping("/{id}")
  public SalesmanResponse update(
      @PathVariable Long id, @RequestBody SalesmanUpdateRequest request) {
    log.debug("Received request to update salesman id={}", id);
    SalesmanResponse updated =
        salesmanService
            .update(id, request)
  ...
  public ResponseEntity<Void> softDelete(@PathVariable Long id) {
    ...
    salesmanService.softDelete(id).orElseThrow(() -> ResourceNotFoundException.salesmanById(id));
