...
public interface SalesmanRepository extends JpaRepository<Salesman, Long> {
  ...
  @Query(
      "SELECT s FROM Salesman s JOIN FETCH s.user WHERE s.deletedFlag = 'N' AND s.user.deletedFlag = 'N'")
  List<Salesman> findAllWithUser();

  @Query(
      "SELECT s FROM Salesman s JOIN FETCH s.user WHERE s.salesmanId = :id AND s.deletedFlag = 'N' AND s.user.deletedFlag = 'N'")
  ...
  @Query(
      "SELECT s FROM Salesman s WHERE s.user.userId = :userId AND s.deletedFlag = 'N' AND s.user.deletedFlag = 'N'")
