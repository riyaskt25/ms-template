...
public class SalesmanCreateRequest {
    ...
    @Schema(description = "Salesman email address", example = "ahamed.khan@example.com", maxLength = 150, requiredMode = Schema.RequiredMode.REQUIRED)
    private String emailAddress;

    @NotBlank(message = "{validation.salesman.mobileNumber.required}")
    @Size(max = 20, message = "{validation.salesman.mobileNumber.size}")
    @Schema(description = "Salesman mobile number", example = "+971555010201", maxLength = 20, requiredMode = Schema.RequiredMode.REQUIRED)
