...
public class CompanyController implements CompanyApi {
  ...
  public CompanyPageResponse findAll(@Valid @ModelAttribute CompanyOffsetListQuery query) {
    log.debug(
        "Received request to fetch companies (offset pagination): page={}, size={}, sortBy={}, sortDirection={}, includeSalesmen={}",
        query.getPage(),
        query.getSize(),
        query.getSortBy(),
        query.getSortDirection(),
        query.getIncludeSalesmen());
  ...
  public CompanyPageResponse findAllLazy(@Valid @ModelAttribute CompanyListQuery query) {
    log.debug(
        "Received request to fetch companies (cursor lazy loading): cursor={}, limit={}, sortBy={}, sortDirection={}, includeSalesmen={}",
        query.getCursor(),
        query.getLimit(),
        query.getSortBy(),
        query.getSortDirection(),
    ...
    log.info(
        "Fetched companies (cursor mode): limit={}, returned={}, hasNext={}, nextCursor={}",
        companies.getLimit(),
        companies.getData().size(),
        companies.isHasNext(),
  ...
  @Override
  @PostMapping
  public ResponseEntity<CompanyWriteResponse> create(
      @Valid @RequestBody CompanyCreateRequest request) {
    log.debug(
        "Received request to create company registrationNumber={}",
  ...
  @Override
  @PutMapping("/{id}")
  public CompanyWriteResponse update(
      @Positive(message = "{validation.common.id.positive}") @PathVariable Long id,
      @Valid @RequestBody CompanyUpdateRequest request) {
    log.debug("Received request to update company id={}", id);
    CompanyWriteResponse updated =
        companyService
            .update(id, request)
            .orElseThrow(() -> ResourceNotFoundException.companyById(id));
  ...
  public ResponseEntity<Void> softDelete(
    ...
    companyService.softDelete(id).orElseThrow(() -> ResourceNotFoundException.companyById(id));
  ...
  public CompanyWriteResponse decideStatus(
      @Positive(message = "{validation.common.id.positive}") @PathVariable Long id,
      @Valid @RequestBody CompanyStatusDecisionRequest request) {
    log.debug(
        "Received status decision for company id={} targetStatus={}", id, request.getStatus());
    CompanyWriteResponse updated =
        companyService
            .decideStatus(id, request)
            .orElseThrow(() -> ResourceNotFoundException.companyById(id));
