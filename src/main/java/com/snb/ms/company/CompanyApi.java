...
public interface CompanyApi {
  ...
  @Operation(
      ...
      description =
          "Returns active company records using offset-based pagination. Best for small to medium datasets or when random access to pages is needed.")
  @CommonApiParameters
  @Parameters({
    ...
    @Parameter(
        name = "sortBy",
        description =
            "Sort fields (comma-separated). Defaults to companyId ASC when omitted. Allowed fields: companyId, registrationNumber, companyStatus, companyType, emailAddress, mobileNumber, createdAt, updatedAt.",
        example = "companyStatus,registrationNumber"),
    @Parameter(
        name = "sortDirection",
        description =
            "Sort directions. Provide a single value (e.g. DESC) to apply to all sortBy fields, or comma-separated values matching the sortBy count. Allowed values: ASC, DESC. Defaults to ASC when omitted.",
        example = "ASC,DESC"),
    @Parameter(
        name = "includeSalesmen",
        description = "Set true to include associated salesmen in each company. Defaults to false.",
        example = "false"),
    @Parameter(
        name = "registrationNumber",
        description = "Optional registration number contains filter",
        example = "REG-2026"),
    @Parameter(
        name = "companyStatus",
        description = "Optional company status contains filter",
        example = "ACTIVE"),
    @Parameter(
        name = "companyType",
        description = "Optional company type contains filter",
        example = "DEALER"),
    @Parameter(
        name = "emailAddress",
        description = "Optional email contains filter",
        example = "company@example.com"),
    @Parameter(
        name = "mobileNumber",
        description = "Optional mobile contains filter",
        example = "+971555")
  })
  @ApiResponses({
    @ApiResponse(
  ...
  @Operation(
      ...
      description =
          "Returns active company records using cursor-based lazy loading. Recommended for large datasets or continuous scrolling. Provides efficient, deterministic pagination.")
  @CommonApiParameters
  @Parameters({
    @Parameter(
        name = "cursor",
        description =
            "Opaque cursor token from previous response's lazyLoading.nextCursor. Omit for first request.",
        example = "eyJjb21wYW55SWQiOiIxMjAifQ"),
    @Parameter(name = "limit", description = "Lazy-loading batch size (max 200)", example = "20"),
    @Parameter(
        name = "sortBy",
        description =
            "Sort fields (comma-separated). Defaults to companyId ASC when omitted. companyId is always appended as tie-breaker when missing. Allowed fields: companyId, registrationNumber, companyStatus, companyType, emailAddress, mobileNumber, createdAt, updatedAt.",
        example = "companyStatus,registrationNumber"),
    @Parameter(
        name = "sortDirection",
        description =
  ...
  @Parameters({
    ...
    @Parameter(
        name = "includeSalesmen",
        description =
            "Set true to include associated salesmen for this company. Defaults to false.",
        example = "false")
  })
  @ApiResponses({
    @ApiResponse(
  ...
  CompanyResponse findById(
      @Positive(message = "{validation.common.id.positive}") Long id, Boolean includeSalesmen);
  ...
  @ApiResponses({
    @ApiResponse(responseCode = "204", description = "Company soft-deleted successfully"),
    @ApiResponse(
        responseCode = "404",
        description = "Company not found",
        content = @Content(schema = @Schema(implementation = BaseResponseDTO.class)))
  ...
  CompanyWriteResponse decideStatus(
      @Positive(message = "{validation.common.id.positive}") Long id,
