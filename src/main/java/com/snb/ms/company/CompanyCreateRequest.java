...
@NoArgsConstructor
@AllArgsConstructor
@Schema(description = "Company creation request payload")
public class CompanyCreateRequest {
    ...
    @Schema(description = "Company email address", example = "company@example.com", maxLength = 150, requiredMode = Schema.RequiredMode.REQUIRED)
    private String emailAddress;

    @NotBlank(message = "{validation.company.mobileNumber.required}")
    @Size(max = 20, message = "{validation.company.mobileNumber.size}")
    @Schema(description = "Company mobile number", example = "+971555010101", maxLength = 20, requiredMode = Schema.RequiredMode.REQUIRED)
