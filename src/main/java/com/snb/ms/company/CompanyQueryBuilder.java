...
/**
 * Query builder utility for Company entity.
 * Encapsulates pagination, sorting, and filtering logic.
...
public final class CompanyQueryBuilder {
    ...
    private static final Map<String, Class<? extends Comparable<?>>> FIELD_TYPES = Map.of(
    ...
    /**
     * Builds a Pageable for cursor-based lazy loading.
     * Supports the same multi-sort inputs as offset mode and appends companyId as tie-breaker when missing.
     */
    ...
    /**
     * Builds a Sort from comma-separated field and direction strings.
     * Validates fields against ALLOWED_SORTS whitelist.
     */
    ...
    /**
     * Builds sort orders for cursor mode and guarantees deterministic traversal using companyId tie-breaker.
     */
    public static Sort buildCursorSort(String sortBy, String sortDirection) {
        ...
        boolean hasCompanyId = orders.stream().anyMatch(order -> "companyId".equals(order.getProperty()));
        if (!hasCompanyId) {
     ...
     */
    public static Specification<Company> buildSpecification(String registrationNumber,
     ...
     */
    public static Specification<Company> withCursor(Specification<Company> baseSpec,
                                                    String cursor,
                                                    String sortBy,
    ...
    private static Comparable<?> parseCursorValue(String mappedField, String rawValue) {
        ...
        if (Objects.equals(type, LocalDateTime.class)) {
            try {
                ...
                throw new IllegalArgumentException("Invalid cursor datetime value for field " + mappedField, ex);
            }
        }
        return rawValue;
    }
    ...
    private static Path<? extends Comparable<?>> resolveComparablePath(Root<Company> root, String mappedField) {
    ...
    private static Object extractFieldValue(Company company, String mappedField) {
        return switch (mappedField) {
            ...
            case "user.emailAddress" -> company.getUser() == null ? null : company.getUser().getEmailAddress();
            case "user.mobileNumber" -> company.getUser() == null ? null : company.getUser().getMobileNumber();
            default -> throw new IllegalArgumentException("Unsupported cursor field: " + mappedField);
        };
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    private static Predicate buildComparisonPredicate(jakarta.persistence.criteria.CriteriaBuilder cb,
