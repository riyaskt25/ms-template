...
public interface CompanyRepository
  ...
  @Query(
      "SELECT c FROM Company c JOIN FETCH c.user WHERE c.companyId = :id AND c.deletedFlag = 'N'")
  ...
  @Query(
      "SELECT c FROM Company c WHERE c.registrationNumber = :registrationNumber AND c.deletedFlag = 'N'")
  Optional<Company> findActiveByRegistrationNumber(
      @Param("registrationNumber") String registrationNumber);

  @Query(
      "SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Company c WHERE c.registrationNumber = :registrationNumber AND c.deletedFlag = 'N'")
