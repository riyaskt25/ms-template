...
@OpenAPIDefinition(
    info =
        @Info(
            ...
            version = "v1",
            description =
                "Microservice template API exposing companies, salesmen, admin users, RBAC (roles, privileges, role-privileges, user-roles), and external posts endpoints.",
            contact =
                @Contact(
    ...
    return new OpenAPI()
        .info(
            new io.swagger.v3.oas.models.info.Info()
                ...
                .version("v1")
                .description(
                    "Detailed API contract for companies, salesmen, admin users, RBAC, and external posts integration endpoints."))
        .components(
            new Components()
                .addParameters(
                    "XRequestIdHeader",
                    new Parameter()
                        ...
                        .in("header")
                        .description(
                            "Correlation identifier for distributed tracing. If omitted, the service generates one.")
                        .required(false)
