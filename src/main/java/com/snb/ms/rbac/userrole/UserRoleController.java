...
public class UserRoleController implements UserRoleApi {
  ...
  @Override
  @PostMapping
  public ResponseEntity<UserRolesAggregateResponse> assign(
      @PathVariable Long userId, @Valid @RequestBody UserRoleRequest request) {
    log.debug(
        "Received request to assign roleCodes={} to userId={}", request.getRoleCodes(), userId);
  ...
  @Override
  @PutMapping
  public UserRolesAggregateResponse replace(
      @PathVariable Long userId, @Valid @RequestBody UserRoleRequest request) {
    log.debug(
        "Received request to replace roles with roleCodes={} for userId={}",
