...
public class RoleController implements RoleApi {
  ...
  @Override
  @PostMapping("/bulk")
  public ResponseEntity<RoleBulkResponse> createBulk(
      @Valid @RequestBody RoleBulkCreateRequest request) {
  ...
  public RoleResponse update(
      @PathVariable String roleCode, @RequestBody RoleUpdateRequest request) {
    log.debug("Received request to update role code={}", roleCode);
    RoleResponse updated =
        roleService
            .update(roleCode, request)
            .orElseThrow(() -> ResourceNotFoundException.roleByCode(roleCode));
