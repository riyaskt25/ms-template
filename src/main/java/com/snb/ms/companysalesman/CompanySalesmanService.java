...
public class CompanySalesmanService {
  ...
  public List<CompanySalesmanDto> findAll() {
    ...
    List<CompanySalesmanDto> associations =
        companySalesmanMapper.toDtoList(companySalesmanRepository.findAllActive());
  ...
  public Optional<CompanySalesmanDto> findById(Long id) {
    ...
    Optional<CompanySalesmanDto> result =
        companySalesmanRepository.findActiveById(id).map(companySalesmanMapper::toDto);
  ...
  public List<CompanySalesmanDto> findByCompanyId(Long companyId) {
    ...
    List<CompanySalesmanDto> result =
        companySalesmanMapper.toDtoList(companySalesmanRepository.findByActiveCompanyId(companyId));
  ...
  public List<CompanySalesmanDto> findBySalesmanId(Long salesmanId) {
    ...
    List<CompanySalesmanDto> result =
        companySalesmanMapper.toDtoList(
            companySalesmanRepository.findByActiveSalesmanId(salesmanId));
  ...
  public void createAssociation(Long companyId, Salesman salesman) {
    log.debug(
        "Creating company-salesman association companyId={} salesmanId={}",
        companyId,
    ...
    CompanySalesman entity =
        CompanySalesman.builder()
            .company(company)
            .salesman(salesman)
    ...
    log.info(
        "Created company-salesman association companyId={} salesmanId={}",
        companyId,
        salesman.getSalesmanId());
  }

  @Transactional
  public CompanySalesmanDto create(
      Long companyId, Long salesmanId, CompanySalesmanRequest request) {
    ...
    Company company =
        companyRepository
            ...
            .orElseThrow(
                () -> {
                  log.error(
                      "Company not found for company-salesman create companyId={}", companyId);
                  return ResourceNotFoundException.companyById(companyId);
                });
    Salesman salesman =
        salesmanRepository
            .findActiveById(salesmanId)
            .orElseThrow(
                () -> {
                  log.error(
                      "Salesman not found for company-salesman create salesmanId={}", salesmanId);
    ...
    CompanySalesmanDto created =
        companySalesmanMapper.toDto(companySalesmanRepository.save(entity));
    log.info(
        "Created company-salesman record id={} companyId={} salesmanId={}",
        created.getCompanySalesmanId(),
        companyId,
        salesmanId);
    return created;
  }
  ...
  public Optional<CompanySalesmanDto> update(Long id, CompanySalesmanRequest request) {
    ...
    Optional<CompanySalesmanDto> updated =
        companySalesmanRepository
            .findActiveById(id)
            .map(
                existing -> {
